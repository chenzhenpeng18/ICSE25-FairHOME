...
if __name__ == '__main__':
    ...
    for r in range(repeat_time):
        ...
        clf4 = get_classifier(clf_name, dataset_orig_train.features.shape[1:])
        clf4.fit(dataset_orig_train_new_for_attr3.features, dataset_orig_train_new_for_attr3.labels)
        ...
        pred_de4 = clf4.predict(dataset_orig_test_4.features)

        res = []
        for i in range(len(pred_de1)):
            prob_t = (pred_de1[i]+pred_de2[i]+pred_de3[i]+pred_de4[i])/4
            if prob_t >= 0.5:
                res.append(1)
            else:
                res.append(0)
