...
val_name = "fairhome2_{}_{}.txt".format(clf_name,dataset_used)
...
for r in range(repeat_time):
    ...
    pred_de5 = clf.predict_proba(X_test5)
    pred_de6 = clf.predict_proba(X_test6)
    pred_de7 = clf.predict_proba(X_test7)
    pred_de8 = clf.predict_proba(X_test8)

    res = []
    for i in range(len(pred_de1)):
        prob_t = (pred_de1[i][1] + pred_de2[i][1] + pred_de3[i][1] + pred_de4[i][1]+pred_de5[i][1]+pred_de6[i][1]+pred_de7[i][1]+pred_de8[i][1]) / 8
        if prob_t >= 0.5:
            res.append(1)
        else:
            res.append(0)
