...
val_name = "fairhome3_{}_{}.txt".format(clf_name,dataset_used)
...
for r in range(repeat_time):
    ...
    pred_de = []
    pred_de.append(clf.predict_proba(X_test))
    pred_de.append(clf.predict_proba(X_test2))
    pred_de.append(clf.predict_proba(X_test3))
    pred_de.append(clf.predict_proba(X_test4))
    pred_de.append(clf.predict_proba(X_test5))
    pred_de.append(clf.predict_proba(X_test6))
    pred_de.append(clf.predict_proba(X_test7))
    pred_de.append(clf.predict_proba(X_test8))
    ...
    for i in range(len(pred_de[0])):
        ...
        conf_sum = 0
        for j in range(8):
            conf_list.append(abs(pred_de[j][i][0] - 0.5))
        ...
        prob_t = 0
        for j in range(8):
            prob_t = prob_t + pred_de[j][i][1] * conf_list[j]
        if prob_t >= 0.5:
            res.append(1)
        else:
            res.append(0)
