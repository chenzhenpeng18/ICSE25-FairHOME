...
for r in range(repeat_time):
    ...
    for i in column_train:
        if i != sa0 and i != sa1 and i != sa2:
            tmp_reg = LinearRegression().fit(X_train[[sa0, sa1,sa2]], X_train[i])
            X_test2[i] = X_test[i] - tmp_reg.predict(X_test[[sa0, sa1, sa2]]) + tmp_reg.predict(X_test2[[sa0, sa1, sa2]])
            X_test3[i] = X_test[i] - tmp_reg.predict(X_test[[sa0, sa1, sa2]]) + tmp_reg.predict(X_test3[[sa0, sa1, sa2]])
            X_test4[i] = X_test[i] - tmp_reg.predict(X_test[[sa0, sa1, sa2]]) + tmp_reg.predict(X_test4[[sa0, sa1, sa2]])
            X_test5[i] = X_test[i] - tmp_reg.predict(X_test[[sa0, sa1, sa2]]) + tmp_reg.predict(X_test5[[sa0, sa1, sa2]])
            X_test6[i] = X_test[i] - tmp_reg.predict(X_test[[sa0, sa1, sa2]]) + tmp_reg.predict(X_test6[[sa0, sa1, sa2]])
            X_test7[i] = X_test[i] - tmp_reg.predict(X_test[[sa0, sa1, sa2]]) + tmp_reg.predict(X_test7[[sa0, sa1, sa2]])
            X_test8[i] = X_test[i] - tmp_reg.predict(X_test[[sa0, sa1, sa2]]) + tmp_reg.predict(X_test8[[sa0, sa1, sa2]])
    ...
    for i in range(len(pred_de1)):
        count1 = pred_de1[i] + pred_de2[i] + pred_de3[i] + pred_de4[i] + pred_de5[i] + pred_de6[i] + pred_de7[i] + \
                 pred_de8[i]
        count0 = 8 - count1
        if count1 >= count0:
            res.append(1)
